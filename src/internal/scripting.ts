...
export const Scripting = (() => {
  ...
  function forget(key) {
    if (typeof key !== 'string') {
      throw new TypeError(
        `forget key parameter must be a string (received: ${Util.getType(key)})`
      );
    }
  ...
  function hasVisited(/* variadic */) {
    ...
    logger.debug(
      `[Scripting/hasVisited()]: needles: '${needles}' played: '${played}'`
    );

    for (let i = 0, iend = needles.length; i < iend; ++i) {
      if (!played.includes(needles[i])) {
        logger.debug(
          `[Scripting/hasVisited()]: returning false because !played.includes(needles[i]). needles[i]: ${needles[i]}`
        );
        return false;
      }
    }
  ...
  function memorize(key, value) {
    if (typeof key !== 'string') {
      throw new TypeError(
        `memorize key parameter must be a string (received: ${Util.getType(
          key
        )})`
      );
    }
  ...
  function recall(key, defaultValue) {
    if (typeof key !== 'string') {
      throw new TypeError(
        `recall key parameter must be a string (received: ${Util.getType(key)})`
      );
    }
  ...
  function visited(/* variadic */) {
    ...
    const needles = Array.prototype.concat.apply(
      [],
      arguments.length === 0 ? [State.passage] : arguments
    );
    const played = State.passages;
        ...
        jQuery(document.createElement('script'))
          .one('load abort error', (ev) => {
            ...
            if (ev.type === 'load') {
              ...
              reject(
                new Error(`importScripts failed to load the script "${url}".`)
              );
            }
          })
        ...
        jQuery(document.createElement('link'))
          .one('load abort error', (ev) => {
            ...
            if (ev.type === 'load') {
              ...
              reject(
                new Error(
                  `importStyles failed to load the stylesheet "${url}".`
                )
              );
            }
          })
