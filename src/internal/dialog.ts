...
export const Dialog = (() => {
  ...
  function dialogInit() {
    ...
    */
    ...
    /*
		...
		*/
    const $elems = jQuery(document.createDocumentFragment()).append(
      ...
      '<div id="ui-overlay" class="ui-close"></div>' +
        ...
        `<button id="ui-dialog-close" class="ui-close" tabindex="0" aria-label="${L10n.get(
          'close'
        )}">\uE804</button>` +
        '</div>' +
  ...
  function dialogIsOpen(classNames?) {
    return (
      ...
      (classNames
        ? classNames
            .splitOrEmpty(/\s+/)
            .every((cn) => _$dialogBody.hasClass(cn))
        : true)
    );
  }
  ...
  function _resizeHandler(ev) {
    const top =
      ev && ev.data && typeof ev.data.top !== 'undefined' ? ev.data.top : 50;
