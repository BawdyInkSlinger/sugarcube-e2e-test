...
export const Setting = (() => { // eslint-disable-line no-unused-vars, no-var
	'use strict';
	...
	*******************************************************************************************************************/
	function settingsInit() {
		...
		if (StorageContainer.storage.has('options')) {
			const old = StorageContainer.storage.get('options');
	...
	function settingsSave() {
		...
		return StorageContainer.storage.set('settings', savedSettings);
	}
		...
		*/
		...
		case Types.Range:
			...
			if (!def.hasOwnProperty('step')) {
				...
				function stepValidate(value: number) {
					if (fracDigits > 0) {
