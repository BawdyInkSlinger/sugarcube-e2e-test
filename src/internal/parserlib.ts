  ...
  *******************************************************************************************************************/
  ...
  Wikifier.Parser.add({
    ...
    parseBody(w, macro) {
      ...
      while (
        (w.matchStart = w.source.indexOf(this.match, w.nextMatch)) !== -1
      ) {
        if (!this.parseTag(w)) {
          this.lookahead.lastIndex = w.nextMatch =
  ...
  Wikifier.Parser.add({
    ...
    handler(w: Wikifier) {
      ...
      const text = markup.hasOwnProperty('text')
        ? Wikifier.helpers.evalText(markup.text)
        : link;
    ...
    processDataAttributes(el: Element, tagName) {
      ...
      if (passage !== '') {
        ...
        if (this.mediaTags.includes(tagName)) {
          if (
            (passage as string).slice(0, 5) !== 'data:' &&
            Story.has(passage as string)
          ) {
