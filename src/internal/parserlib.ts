...
Wikifier.Parser.add({
  ...
  handler(w: Wikifier) {
    ...
    w.subWikify(
      jQuery(document.createElement('blockquote')).appendTo(w.output).get(0),
      this.terminator
    );
  },
});
...
Wikifier.Parser.add({
    ...
    if (this.parseTag(w)) {
      ...
      try {
        ...
        if (macro) {
          ...
          if (typeof macro.handler === 'function') {
            ...
            return appendError(
              ...
              `macro <<${name}>> handler function ${
                typeof macro.handler === 'undefined'
                  ? 'does not exist'
                  : 'is not a function'
              }`,
              w.source.slice(matchStart, w.nextMatch)
          ...
          return appendError(
            w.output,
            `child tag <<${name}>> was found outside of a call to its parent macro${
              tags.length === 1 ? '' : 's'
            } <<${tags.join('>>, <<')}>>`,
            w.source.slice(matchStart, w.nextMatch)
          );
        ...
        return appendError(
          w.output,
          `cannot execute ${
            macro && macro.isWidget ? 'widget' : 'macro'
          } <<${name}>>: ${ex.message}`,
          w.source.slice(matchStart, w.nextMatch)
        );
      } finally {
  ...
  parseArgs: (() => {
    ...
    function lexSpace(lexer: Lexer): typeof lexSpace {
    ...
    function lexExpression(lexer: Lexer): typeof lexSpace {
    ...
    function lexDoubleQuote(lexer: Lexer): typeof lexSpace {
      if (slurpQuote(lexer, '"') === EOF) {
    ...
    function lexSingleQuote(lexer: Lexer): typeof lexSpace {
      if (slurpQuote(lexer, "'") === EOF) {
    ...
    function lexSquareBracket(lexer: Lexer): typeof lexSpace {
    ...
    function lexBareword(lexer: Lexer): typeof lexSpace {
    ...
    function parseMacroArgs(rawArgsString: string) {
      ...
      lexer.run().forEach((item) => {
        ...
        switch (item.type) {
          case Item.Error:
            throw new Error(
              `unable to parse macro argument "${arg}": ${item.message}`
            );
          ...
          case Item.Bareword: {
            ...
            // Property access on the settings or setup objects, so try to evaluate it.
              ...
              try {
                ...
                throw new Error(
                  `unable to parse macro argument "${arg}": ${ex.message}`
                );
              }
            }
          ...
          case Item.Expression: {
            ...
            // Evaluate the expression.
              ...
              try {
                ...
                throw new Error(
                  `unable to parse macro argument expression "${arg}": ${ex.message}`
                );
              }
            }
          ...
          case Item.String: {
            ...
            try {
              ...
              throw new Error(
                `unable to parse macro argument string "${arg}": ${ex.message}`
              );
            }
          ...
          case Item.SquareBracket: {
            ...
            if (Object.hasOwn(markup, 'error')) {
              throw new Error(
                `unable to parse macro argument "${arg}": ${markup.error}`
              );
            }

            if (markup.pos < arg.length) {
              throw new Error(
                `unable to parse macro argument "${arg}": unexpected character(s) "${arg.slice(
                  markup.pos
                )}" (pos: ${markup.pos})`
              );
            }
            ...
            if (markup.isLink) {
              ...
              arg.text = Object.hasOwn(markup, 'text')
                ? Wikifier.helpers.evalText(markup.text)
                : arg.link;
              arg.external =
                !markup.forceInternal && Wikifier.isExternalLink(arg.link);
              arg.setFn = Object.hasOwn(markup, 'setter')
                ? Wikifier.helpers.shadowHandler(
                    Scripting.desugar(markup.setter)
                  )
                : null;
              ...
              arg = ((source) => {
                ...
                if (source.slice(0, 5) !== 'data:' && Story.has(source)) {
                  const passage: any = Story.get(source);
...
Wikifier.Parser.add({
  ...
  handler(w: Wikifier) {
    ...
    const text = Object.hasOwn(markup, 'text')
      ? Wikifier.helpers.evalText(markup.text)
      : link;
...
Wikifier.Parser.add({
  ...
  handler(w: Wikifier) {
    switch (w.matchText) {
      case "''":
        w.subWikify(
          jQuery(document.createElement('strong')).appendTo(w.output).get(0),
          "''"
        );
        break;
      ...
      case '//':
        w.subWikify(
          jQuery(document.createElement('em')).appendTo(w.output).get(0),
          '//'
        );
        break;
      ...
      case '^^':
        w.subWikify(
          jQuery(document.createElement('sup')).appendTo(w.output).get(0),
          '\\^\\^'
        );
        break;
      ...
      case '~~':
        w.subWikify(
          jQuery(document.createElement('sub')).appendTo(w.output).get(0),
          '~~'
        );
        break;
...
Wikifier.Parser.add({
  ...
  handler(w: Wikifier) {
    ...
    if (
      css.classes.length === 0 &&
      css.id === '' &&
      Object.keys(css.styles).length === 0
    ) {
      $el.addClass('marked');
    } else {
...
Wikifier.Parser.add({
  ...
  handler(w: Wikifier) {
    ...
    w.subWikify(
      jQuery(document.createElement(`h${w.matchLength}`))
        .appendTo(w.output)
        .get(0),
      this.terminator
    );
...
Wikifier.Parser.add({
  ...
  processAttributeDirectives(el: Element) {
    ...
    Array.from(el.attributes).forEach(({ name, value }) => {
      ...
      if (evalShorthand || name.startsWith('sc-eval:')) {
        ...
        if (newName === 'data-setter') {
          throw new Error(
            `evaluation directive is not allowed on the data-setter attribute: "${name}"`
          );
        }
        ...
        try {
          ...
          throw new Error(
            `bad evaluation from attribute directive "${name}": ${ex.message}`
          );
        }
        ...
        try {
          ...
          throw new Error(
            `cannot transform attribute directive "${name}" into attribute "${newName}"`
          );
        }
      }
    });
  ...
  processDataAttributes(el, tagName) {
    ...
    if (passage !== '') {
      ...
      // Elsewise, assume a link element of some type—e.g., '<a>'.
        ...
        jQuery(el).ariaClick({ one: true }, function (this: Element) {
...
Wikifier.Parser.add({
  ...
  handler(w: Wikifier) {
    ...
    if (tagName) {
      ...
      if (isVoid || terminatorMatch) {
        let output = w.output;
        let el = document.createElement((w.output as any).tagName);
        let debugView;
  ...
  processDataAttributes(el: Element, tagName) {
    let passage: Passage | string = el.getAttribute('data-passage');
    ...
    if (passage !== '') {
      ...
      if (this.mediaTags.includes(tagName)) {
        if ((passage as string).slice(0, 5) !== 'data:' && Story.has((passage as string))) {
          passage = Story.get((passage as string));
          ...
          switch (tagName) {
            ...
            case 'source': {
              ...
              if ($parent.length > 0) {
                ...
                twineTag = `Twine.${
                  parentName === 'picture' ? 'image' : parentName
                }`;
              }
          ...
          if ((passage as any).tags.includes(twineTag)) {
            el[parentName === 'picture' ? 'srcset' : 'src'] =
              (passage as any).text.trim();
          }
        }
      }
      ...
      // Elsewise, assume a link element of some type—e.g., '<a>', '<area>', '<button>', etc.
        ...
        if (Story.has((passage as string))) {
          el.classList.add('link-internal');

          if (Config.addVisitedLinkClass && State.hasPlayed((passage as string))) {
            el.classList.add('link-visited');
          }
        } else {
          el.classList.add('link-broken');
