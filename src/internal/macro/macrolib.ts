...
Macro.add('silently', {
  ...
  handler() {
    ...
    if (Config.debug) {
      ...
      const errList = [...frag.querySelectorAll('.error')].map(
...
Macro.add('type', {
  ...
  handler() {
    ...
    if (speed < 0) {
      return this.error(
        `speed time value must be non-negative (received: ${this.args[0]})`
      );
    }
    ...
    while (options.length > 0) {
      ...
      switch (option) {
        ...
        case 'start': {
          ...
          if (start < 0) {
            throw new Error(
              `start option time value must be non-negative (received: ${value})`
            );
          }
    ...
    if (!TempStateContainer.TempState.macroTypeQueue) {
      // Set up the typing handler queue for all invocations.
      TempStateContainer.TempState.macroTypeQueue = [];
    ...
    const startTyping =
      TempStateContainer.TempState.macroTypeQueue.length === 0;

    // Generate our unique ID.
    const selfId = ++this.self.typeId;

    // Push our typing handler onto the queue.
    TempStateContainer.TempState.macroTypeQueue.push({
      id: selfId,

      handler() {
        const $wrapper = jQuery(document.createElement(elTag)).addClass(
          className
        );
        ...
        if (
          ...
          TempStateContainer.TempState.macroTypeQueue.shift();

          // Run the next typing handler in the queue, if any.
          if (TempStateContainer.TempState.macroTypeQueue.length > 0) {
            TempStateContainer.TempState.macroTypeQueue.first().handler();
          }
        ...
        const typeNode = function typeNode() {
          const typeNodeMember = function typeNodeMember(typeIntervalId?) {
            ...
            if (
              ...
              if (
                TempStateContainer.TempState.macroTypeQueue &&
                TempStateContainer.TempState.macroTypeQueue.length > 0 &&
                TempStateContainer.TempState.macroTypeQueue.first().id ===
                  selfId
              ) {
                TempStateContainer.TempState.macroTypeQueue.shift();
              }
          ...
          const typeNodeMemberId = setInterval(
            () => typeNodeMember(typeNodeMemberId),
            speed
          );
        };
    ...
    if (startTyping) {
      if (Engine.isPlaying()) {
        $(document).one(`:passageend${namespace}`, () =>
          TempStateContainer.TempState.macroTypeQueue.first().handler()
        );
      } else {
        TempStateContainer.TempState.macroTypeQueue.first().handler();
        ...
        switch (this.payload[i].name) {
          case 'else':
            if (this.payload[i].args.raw.length > 0) {
              ...
              return this.error(
                `<<else>> does not accept a conditional expression (perhaps you meant to use <<elseif>>), invalid: ${this.payload[i].args.raw}`
              );
            }
...
Macro.add('switch', {
  ...
  handler() {
    ...
    for (/* declared previously */ i = 1; i < len; ++i) {
      switch (this.payload[i].name) {
        ...
        default:
          if (this.payload[i].args.length === 0) {
            return this.error(
              `no value(s) specified for <<${this.payload[i].name}>> (#${i})`
            );
          }
          break;
      }
    ...
    for (/* declared previously */ i = 1; i < len; ++i) {
      ...
      // Case test(s).
      if (
        this.payload[i].name === 'default' ||
        this.payload[i].args.some((val) => val === result)
      ) {
        success = true;
        new Wikifier(this.output, this.payload[i].contents);
        break;
...
Macro.add('for', {
  ...
  hasRangeRe: new RegExp(
    `^\\S${Patterns.anyChar}*?\\s+range\\s+\\S${Patterns.anyChar}*?$`
  ),
  rangeRe: new RegExp(
    `^(?:State\\.(variables|temporary)\\.(${Patterns.identifier})\\s*,\\s*)?State\\.(variables|temporary)\\.(${Patterns.identifier})\\s+range\\s+(\\S${Patterns.anyChar}*?)$`
  ),
  threePartRe: /^([^;]*?)\s*;\s*([^;]*?)\s*;\s*([^;]*?)$/,
    ...
    try {
      ...
      if (init) {
        try {
          ...
          return this.error(
            `bad init expression: ${typeof ex === 'object' ? ex.message : ex}`
          );
        }
      }
      ...
      while (evalJavaScript(condition)) {
        if (--safety < 0) {
          return this.error(
            `exceeded configured maximum loop iterations (${Config.macros.maxLoopIterations})`
          );
        }
        ...
        if (TempStateContainer.TempState.break != null) {
          // lazy equality for null
          if (TempStateContainer.TempState.break === 1) {
            TempStateContainer.TempState.break = null;
          } else if (TempStateContainer.TempState.break === 2) {
            TempStateContainer.TempState.break = null;
            break;
          }
        }
        ...
        if (post) {
          try {
            ...
            return this.error(
              `bad post expression: ${typeof ex === 'object' ? ex.message : ex}`
            );
          }
        }
      }
    ...
    switch (typeof value) {
      ...
      case 'object':
        if (Array.isArray(value)) {
          ...
          throw new Error(
            `unsupported range expression type: ${Util.toStringTag(value)}`
          );
        }
        break;
...
Macro.add(['break', 'continue'], {
  ...
  handler() {
    if (this.contextHas((ctx) => ctx.name === 'for')) {
      TempStateContainer.TempState.break = this.name === 'continue' ? 1 : 2;
    } else {
      return this.error(
...
Macro.add(['button', 'link'], {
  ...
  handler() {
    if (this.args.length === 0) {
      return this.error(
        `no ${this.name === 'button' ? 'button' : 'link'} text specified`
      );
    }

    const $link = jQuery(
      document.createElement(this.name === 'button' ? 'button' : 'a')
    );
    let passage;
  ...
  handler() {
    ...
    for (let i = 1; i < len; ++i) {
      ...
      if (payload.name === 'option') {
        ...
        if (payload.args.length === 0) {
          return this.error(
            `no arguments specified for <<${payload.name}>> (#${tagCount.option})`
          );
        }
        ...
        if (isSelected) {
          if (config.autoselect) {
            ...
            return this.error(
              `multiple selected keywords specified for <<${payload.name}>> (#${
                selectedIdx + 1
              } & #${tagCount.option})`
            );
          }
      ...
      // <<optionsfrom expression>>
        ...
        if (payload.args.full.length === 0) {
          return this.error(
            `no expression specified for <<${payload.name}>> (#${tagCount.optionsfrom})`
          );
        }
        ...
        if (result instanceof Array || result instanceof Set) {
          ...
          result.forEach((val, key) =>
            options.push({ label: String(key), value: val })
          );
        } else {
          ...
          if (oType !== 'Object') {
            return this.error(
              `expression must yield a supported collection or generic object (object type: ${oType})`
            );
          }
    ...
    if (selectedIdx === -1) {
      ...
      if (config.autoselect) {
        ...
        const curValueIdx = options.findIndex((opt) =>
          sameValueZero(opt.value, curValue)
        );
        ...
        jQuery(document.createElement('a'))
          ...
          .ariaClick(
            ...
            this.createShadowWrapper(function () {
              ...
              $this
                .empty()
                .wikiWithOptions({ profile: 'core' }, options[cycleIdx].label);
...
Macro.add(['numberbox', 'textbox'], {
  ...
  handler() {
    ...
    if (asNumber && Number.isNaN(defaultValue)) {
      return this.error(
        `default value "${this.args[1]}" is neither a number nor can it be parsed into a number`
      );
    }
...
Macro.add('radiobutton', {
  ...
  handler() {
    ...
    /*
        ...
        */
    if (!TempStateContainer.TempState.hasOwnProperty(this.name)) {
      TempStateContainer.TempState[this.name] = {};
    }

    if (!TempStateContainer.TempState[this.name].hasOwnProperty(varId)) {
      TempStateContainer.TempState[this.name][varId] = 0;
    }
    ...
    jQuery(el)
      .attr({
        id: `${this.name}-${varId}-${TempStateContainer.TempState[this.name][
          varId
        ]++}`,
        name: `${this.name}-${varId}`,
        type: 'radio',
  ...
  handler() {
    ...
    if (passage == null) {
      ...
      if (this.name === 'back') {
        ...
        if (momentIndex === -1) {
          return this.error(
            `cannot find passage "${passage}" in the current story history`
          );
        }
      }
    }
      ...
      .append(
        $image ||
          document.createTextNode(
            text || L10n.get(`macro${this.name.toUpperFirst()}Text`)
          )
      )
      .appendTo(this.output);
  },
...
*******************************************************************************************************************/
if (Has.audio) {
  const errorOnePlaybackAction = (cur, prev) =>
    `only one playback action allowed per invocation, "${cur}" cannot be combined with "${prev}"`;
    ...
    handler() {
        ...
        switch (arg) {
          ...
          case 'fadeoverto':
            ...
            if (args.length < 2) {
              ...
              return this.error(
                `fadeoverto missing required ${errors.join(' and ')} value${
                  errors.length > 1 ? 's' : ''
                }`
              );
            }
      ...
      try {
        ...
        if (passage != null) {
          ...
          selected.off(nsEnded).one(nsEnded, () => {
        ...
        SimpleAudio.tracks.add(
          ...
          this.args.slice(1).map((source) => {
            ...
            if (oldFmtRe.test(source)) {
              ...
              if (Config.debug) {
                return this.error(
                  `track ID "${id}": format specifier migration required, "format:formatId;" \u2192 "formatId|"`
                );
              }
  ...
  Macro.add('createplaylist', {
    ...
    handler() {
      ...
      for (let i = 1, len = this.payload.length; i < len; ++i) {
        ...
        const trackObj: { id: string; own: boolean; volume: number } = {
          id: String(this.payload[i].args[0]).trim(),
          own: undefined,
          volume: undefined,
        }; // BIS added `own: undefined, volume: undefined`
  ...
  registerInterval(callback, delay) {
    ...
    timerId = setInterval(() => {
      ...
      try {
        ...
        if (TempStateContainer.TempState.hasOwnProperty('repeatTimerId')) {
    ...
    this.self.registerTimeout(
      this.createShadowWrapper((item) => {
        ...
        if (Config.debug && item.name === 'next') {
          $output = jQuery(
            new DebugView($output[0], 'macro', item.name, item.source).output // eslint-disable-line no-param-reassign
          );
        }
...
Macro.add('widget', {
  ...
  handler() {
    ...
    try {
      const widgetDef = {
        tags: undefined, // <-- added by BIS
        isWidget: true,
        handler: (function (widgetCode) {
          return function () {
            const shadowStore: { _args?: any; _contents?: any; $args?: any } =
              {};
      ...
      return this.error(
        `cannot create widget macro "${widgetName}": ${ex.message}`
      );
    }
  },
});
