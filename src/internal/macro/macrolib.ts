...
Macro.add('type', {
    ...
    handler() {
        ...
        const startTyping = TempStateContainer.TempState.macroTypeQueue.length === 0;
                    ...
                    const typeNodeMember = function typeNodeMember(typeIntervalId?) {
                        ...
                        if (
                            ...
                            if (
                                ...
                                && TempStateContainer.TempState.macroTypeQueue.first().id === selfId
                            ) {
                                TempStateContainer.TempState.macroTypeQueue.shift();
                            }
        ...
        if (startTyping) {
            if (Engine.isPlaying()) {
                $(document).one(`:passageend${namespace}`, () => TempStateContainer.TempState.macroTypeQueue.first().handler());
            }
            else {
                TempStateContainer.TempState.macroTypeQueue.first().handler();
            }
    ...
    handleFor(payload, init, condition, post) {
        ...
        try {
            ...
            while (evalJavaScript(condition)) {
                ...
                if (TempStateContainer.TempState.break != null) { // lazy equality for null
    ...
    handler() {
        ...
        for (let i = 1; i < len; ++i) {
            ...
            if (payload.name === 'option') {
                ...
                const option: {label: string, value: string} = { label: String(payload.args[0]), value: undefined }; // BIS: added `value: undefined`
        ...
        $link
            ...
            .ariaClick({
                ...
                () => {
                    ...
                    if (transition) {
                        triggerTimeout(`<<${this.name}>> macro transition: ${this.payload[0].contents}`, () => $insert.removeClass(`macro-${this.name}-in`), Engine.minDomActionDelay);
                    }
                }
            ))
            .appendTo(this.output);
    ...
    handler() {
        ...
        if (autofocus) {
            ...
            postdisplay[`#autofocus:${el.id}`] = task => {
                ...
                triggerTimeout(`<<${this.name}>> macro: ${el.value}`, () => el.focus(), Engine.minDomActionDelay);
            };
        }
    }
});
...
Macro.add('radiobutton', {
    ...
    handler() {
        ...
        */
        jQuery(el)
            .attr({
                id: `${this.name}-${varId}-${TempStateContainer.TempState[this.name][varId]++}`,
                name: `${this.name}-${varId}`,
                type: 'radio',
                tabindex: 0 // for accessiblity
            })
    ...
    handler() {
        ...
        */
        if (autofocus) {
            ...
            postdisplay[`#autofocus:${el.id}`] = task => {
                delete postdisplay[task]; // single-use task
                triggerTimeout(`<<textarea>> macro: ${el.textContent}`, () => el.focus(), Engine.minDomActionDelay);
            };
        }
    }
});
    ...
    Macro.add('createplaylist', {
        ...
        handler() {
            ...
            for (let i = 1, len = this.payload.length; i < len; ++i) {
                ...
                const trackObj: {id: string, own: boolean, volume: number} = { id: String(this.payload[i].args[0]).trim(), own: undefined, volume: undefined }; // BIS added `own: undefined, volume: undefined`
...
Macro.add('done', {
    ...
    handler() {
        ...
        triggerTimeout(`<<done>> macro: ${contents}`, 
...
Macro.add('goto', {
    handler() {
        ...
        */
        triggerTimeout(`<<goto>> macro: ${this.args[0]}`, () => Engine.play(passage), Engine.minDomActionDelay);
    }
});
...
Macro.add('repeat', {
    ...
    handler() {
        ...
        this.self.registerInterval(this.createShadowWrapper(() => {
            ...
            if (transition) {
                triggerTimeout(`<<repeat>> macro transition: ${this.args}`, () => $output.removeClass('macro-repeat-in'), Engine.minDomActionDelay);
            }
        }), delay);
    },
...
Macro.add('timed', {
    ...
    handler() {
        ...
        this.self.registerTimeout(this.createShadowWrapper(item => {
            ...
            if (transition) {
                triggerTimeout("<<timed>> macro: transition", () => $output.removeClass('macro-timed-in'), Engine.minDomActionDelay);
            }
        }), items);
    },
    ...
    registerTimeout(callback, items) {
        ...
        const worker = function () {
            ...
            if ((nextItem = items.shift()) != null) { // lazy equality for null
                timerId = triggerTimeout(`<<timed>> macro: ${nextItem.content}`, worker, nextItem.delay);
                timers.add(timerId);
            }
