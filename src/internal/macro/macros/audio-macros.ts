		...
		Macro.add('audio', {
			handler() {
				...
				try {
					...
					return this.error((ex as Error).message);
				}
				...
				try {
					...
					return this.error(`error executing action: ${(ex as Error).message}`);
				}
			}
		});
		...
		Macro.add('createplaylist', {
			...
			handler() {
				...
				for (let i = 1, len = this.payload.length; i < len; ++i) {
					...
					const trackObj: {id: string, own: boolean, volume: number} = { id : String(this.payload[i].args[0]).trim(), own: undefined, volume: undefined };
