...
class Wikifier {
  ...
  declare getValue: (...args: any[]) => any;
  declare setValue: (...args: any[]) => any;
  declare parse: (...args: any[]) => any;
  declare evalExpression: (...args: any[]) => any;
  declare evalStatements: (...args: any[]) => any;
  declare textPrimitives: typeof Patterns;
  declare source: string;
  declare options: any;
  declare nextMatch: number;
  declare _rawArgs: string;
  ...
  declare static Option: {
    length: number;
    options: () => any;
    clear: void;
    get: (idx: number) => any;
    pop: () => any;
    push: (option: { nobr: boolean }) => any;
  };

  declare static helpers: any & {
  ...
  constructor(destination, source, options?) {
    ...
    try {
      ...
      if (
        _callDepth === 1 &&
        (Object.hasOwn(this.options, 'cleanup') && this.options.cleanup != null // lazy equality for null
  ...
  subWikify(output, terminator?, options?) {
    ...
    do {
      ...
      if (
        terminatorMatch &&
        (!parserMatch || terminatorMatch.index <= parserMatch.index)
      ) {
  ...
  outputText(destination, startPos, endPos) {
    jQuery(destination).append(
      document.createTextNode(this.source.substring(startPos, endPos))
    );
  }
  ...
  /*
		...
		*/
  static wikifyEval(text: string): DocumentFragment {
  ...
  /*
		...
		*/
  static createInternalLink(destination: OutputDestination | JQuery, passage: string, text: string, callback: () => void): HTMLAnchorElement {
  ...
  /*
		...
		*/
  static createExternalLink(destination: OutputDestination, url: string, text?: string): HTMLAnchorElement {
    const $link = jQuery(document.createElement('a'))
      .attr('target', '_blank')
...
Object.defineProperty(Wikifier, 'Parser', {
  value: (() => {
    ...
    const _parsers: { name: unknown; profiles: unknown[]; match: unknown }[] =
      [];

    // Parser profiles object.
    let _profiles: object;
    ...
    function profilesCompile() {
      ...
      const core = all.filter(
        (parser) =>
          !Array.isArray(parser.profiles) || parser.profiles.includes('core')
      );
      ...
      _profiles = Object.freeze({
        all: {
          ...
          parserRegExp: new RegExp(
            all.map((parser) => `(${parser.match})`).join('|'),
            'gm'
          ),
        },
        core: {
          parsers: core,
          parserRegExp: new RegExp(
            core.map((parser) => `(${parser.match})`).join('|'),
            'gm'
          ),
        },
      });
    ...
    function profilesIsEmpty() {
      return (
        typeof _profiles !== 'object' || Object.keys(_profiles).length === 0
      );
    }
...
Object.defineProperties(Wikifier.helpers, {
  inlineCss: {
    value: (() => {
      ...
      const idOrClassRe = new RegExp(
        `(${Patterns.cssIdOrClassSigil})(${Patterns.anyLetter}+)`,
        'g'
      );
  ...
  shadowHandler: {
    value: (() => {
      ...
      function getMacroContextShadowView(): string[] {
      ...
      function helperShadowHandler(code) {
        ...
        getMacroContextShadowView().forEach((varName: string) => {
